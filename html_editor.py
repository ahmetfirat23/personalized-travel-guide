...
def create_guide(cities, db, pass_name, flight_list):
    ...
    for i in range(len(cities)):
        ...
        city_loc = geolocator.geocode(city).point
        m = folium.Map(location=[city_loc.latitude, city_loc.longitude], tiles='openstreetmap', zoom_start=10)
        city_data = cities_table[city]
        city_name_h2 = soup.find(id='city-{}'.format(i)).find('h2', {'class': 'city-name'})
        city_name_h2.append(city.title())

        str_template = ''
        for j in range(len(city_data['names'])):
            try:
                point = geolocator.geocode(city_data['names'][j][0]).point
                folium.Marker([point.latitude, point.longitude], popup=city_data['names'][j][0]).add_to(m)
            except:
                pass
            spot_template = ''
            if j % 2 == 0:
            ...
            for review in review_list:
                if len(review) > 500:
                    ...
                    script.write(
                        '''function readMore{0}(){{let dots = document.getElementById('dots-{0}');let more = document.getElementById('more-{0}');let btnText = document.getElementById('more-btn-{0}');'''.format(
                            hashed) + \
                    ...
                    review = review[
                             :500] + "<span id='dots-{0}' style='display:inline'>...</span><span id='more-{0}' ".format(
                        hashed) + "style='display:none'>" + review[500:] + "</span>" + \
            ...
            if city_data['abouts'][j][0] is not None:
                ...
                if j % 2 == 1:
                    spot_template += ''' gray'''
                spot_template += ''''><span class='about-h'>About</span><br> <span class='about'>{}</span>'''.format(
                    city_data['abouts'][j][0])
            spot_template += '</div>'
        ...
        if flight_list[i] != -1 and flight_list[i] is not None:
            ...
            for flight in flight_list[i]:
                ...
                flight_template += '"><span class="f_info">Departure: {}<br>Arrival: {}</span><a class="f_code" href="https://www.google.com/search?q={}">Flight code: {}</a></div>'.format(
                    flight[0], flight[1], flight[2], flight[2])
            flight_template += '</div>'
            str_template += flight_template

        m.save(f'site_template/maps/{city}.html')
        str_template = f'<iframe id="{city}" src="maps/{city}.html" width=60vw height=40vh></iframe>' + str_template
